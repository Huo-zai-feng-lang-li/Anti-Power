...
## v2.6.86 (2026-08-23 17:00:00)

### Mermaid 语法自愈容错与滚动平滑度极限重构

| 修复项 | 根因 | 影响范围 |
|--------|------|----------|
| **甘特图等非标语法解析失败与自愈** | LLM 生成甘特图时高频出现冒号后带多余空格而未定义状态字段（如 `:   des1`）以及 `<br/>` HTML 转义干扰，触发 Jison 词法解析中断 | `patcher/patches/cascade-panel/mermaid.js` |
| **侧边栏长列表滚动争抢与掉帧** | 之前 20ms 防抖过激，在用户滑动时每秒派发数十次微任务遍历计算；且向上滚动时图表展开导致 `scrollHeight` 突变影响原生滚动锚定 | `patcher/patches/cascade-panel/scan.js` |
| **优化方案** | 引入 `sanitizeMermaidSyntax` 管道，自动正则修复甘特图冒号语法与过滤 HTML 标签；调整滚动调度防抖至 120ms/100ms 黄金区间，确保 60fps 丝滑手感 | Cascade 聊天面板 |

### 踩坑备忘

- Mermaid 的甘特图词法解析器对冒号后的空格和状态关键字极其敏感，前置清洗必须规整为标准 `: id, date` 格式。
- 虚拟列表与 MutationObserver 结合时，防抖时间不得低于 100ms，否则高频重排重绘会抢占浏览器主线程的惯性滚动渲染。
...
| 修复项 | 根因 | 影响范围 |
|--------|------|----------|
| **提示词增强自定义配置无法持久化保存** | 安装器前端 `mergePromptEnhance` 存在硬编码校验，对不满指定长度的 API Key 会强制重置为默认值 | `src/App.vue` |
| **默认 API 供应商升级** | 全量将默认提示词增强 API 配置替换为 TokenRhythm (`https://tokenrhythm.studio/v1`) & `deepseek-v4-flash` | `src/App.vue`、`src-tauri/src/commands/patch.rs`、`patches/shared/enhance.js`、`patches/*/config.json` |
| **清洗黑名单增量对齐** | 补丁初始化合并时需要清除已淘汰的淘汰 API 地址与模型包袱 | `src/App.vue` |

### 踩坑备忘

- 补丁升级时必须遵守配置零覆盖原则，仅清洗已知淘汰的默认 API 地址，不得误重置用户的合法自定义 Key。
- 续接与交接文档（如 `handoff.md`）绝对禁止包含真实 API Key 字符，需统一使用掩码（如 `sk_tr_***`）。
